...
let limite = 15;
let pagina = 1;
//SE LLAMA AL METODO DE LA MODAL DE BOOTSTRAP
  ...
  fetch(APIespecies + "listarEspecies")
    ...
    .then((res) => {
      ...
      res.especie.map((especie) => {
        let options =`<option value="${especie.idespecie_ciudadano}">${especie.nombre}</option>` + "</br>";
...
//ENVIAR DATOS POR MEDIO DEL FORMULARI, REQUEST CON PAYLOAD ES LA INFORMACION DE LOS FORMS
//POST
frmCityzen.addEventListener("submit", (e) => {
  e.preventDefault(); //EVITA EL EVENTO POR DEFECTO EN LOS FORMULARIOS
  if (accionForm == "agregar") {
    fetch(api + "crearCiudadano", {
      method: "POST",
      //SE CONFIGURA EL HEADER, HEADER DE PETICION, LLEVA CONFIGURACION, TIENE ARCHIVOS DE JS A JSON
      headers: {
...
function obtenerID(id, traerAccion) {
  if (traerAccion === "editar") {
    ...
    idFila = id;
    let respuesta = window.confirm(`¿Desea borrar el registro con el id: ${idFila}?`);
    if (respuesta) {
